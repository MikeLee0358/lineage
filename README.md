# 天堂衝裝模擬器

## DEMO 網址
  https://lineage-v2.netlify.app/#/pageGame

## 目的 
  個人的 Web 技術練功場，記錄階段性成長。

## 功能
  - 點擊卷軸 -> 要強化的防具 / 武器 -> 成功 or 失敗。
  - 角色講話時觸發不同事件。（強化機率上升、下降、無效化、冷笑話..）。
  
  - v2 (軟體業工作之後):
    - 導入 ESCSS，優化整體架構。
    - 單一角色，專注衝裝備功能。
    - 使用 playwright 做 E2E 測試。

  - v1 (軟體業工作之前):
    - 多種角色，對應不同主題音樂。
    - 使用 Cypress 做 E2E 測試。

## 備註
  v1 只剩下靜態頁面供回憶，Git 紀錄不再此專案，且被早期的我刪除了..。

## 使用技術
  - ESCSS。
  - Cypress、Playwright。
  - CSS、SCSS、Tailwind。
  - Vue 3、Vue router、Pinia。

## 完成 v1 時...
  - [心得文章](https://medium.com/@cspkno005/my-first-side-project-%E5%A4%A9%E5%A0%82%E8%A1%9D%E8%A3%9D%E6%A8%A1%E6%93%AC%E5%99%A8-277182f2962f)
  - [巴哈姆特討論區的評價](https://forum.gamer.com.tw/C.php?bsn=842&snA=714014)
    - 被 天堂題材 youtuber `電玩洛克` 詢問，覺得很開心。
  - [遊戲大亂鬥討論區的評價](https://www.league-funny.com/gaming/article-340123)

## 回顧 v2 後...
  透過整理此 README，才驚訝的發現自己成長了這麼多了...
  

## 資料參考
  - Google、ChatGPT、天堂韓服官方、遊戲主程式。｀
  - 王子：https://leonsway.blogspot.com/2006/06/20060617_20.html
  - 騎士：https://bbs.mychat.to/sindex.php?t361225.html
  - 天堂Ｗ影片: https://www.youtube.com/watch?v=EGql7OgWzzE
  - 衝裝機率: https://forum.gamer.com.tw/Co.php?bsn=00842&sn=5173520
  - Daisy 愛自學(音頻混音)：https://www.daisylove3c.com/
  - J.J.'s Blogs(天堂私服架設)：https://morosedog.gitlab.io/private-lineage-20210712-private-lineage-0/
const UnitTest = {
  ...
  in: {
    ...
    useTypeMode(input, type, errMsg) {
      ...
      if (UnitTest.in.reuse.getNewType(input) !== type) {
        ...
        const defaultErrMsg = `❌ typeof ${fixTextInLogInput} === ${fixTextInLogType}, ✅ expects type '${getNewType}' to be ${fixTextInLogType}`;
  ...
  out: {
     ...
     * unitTest(1, 'object', 'foo') // error &  error message 'foo'
     *
     * // operator mode
     * unitTest(1, '<', 5)
     * unitTest(1, '<=', 5)
     ...
     * unitTest(1, '===', 1)
     * unitTest(1, '!==', 2)
     * unitTest(1, '>=', 100, 'foo') // error & error message 'foo'
     * ```
     */

...
test.describe('mouse scroll testing', () => {
  test('F5 stop function test', async ({ page }) => {
    ...
    await expect(page.locator('[id="🏠page-demo__X2"]')).toContainText('請選擇一種防具。');
  });
  ...
  test('armor to +1', async ({ page }) => {
    ...
    await expect(page.locator('[id="🏠page-demo__X2"]')).toContainText('請選擇一種防具。');
    await page.locator('li:nth-child(9)').click();
    await expect(page.locator('id=🏠page-demo__X2')).toContainText('+0 精靈盾牌 一瞬間發出 銀色的 光芒。');
  ...
  test('armor to -1', async ({ page }) => {
    ...
    await expect(page.locator('[id="🏠page-demo__X2"]')).toContainText('請選擇一種防具。');
    await page.locator('li:nth-child(10)').click();
    await expect(page.locator('id=🏠page-demo__X2')).toContainText('+0 力量手套 一瞬間發出 黑色的 光芒。');
  ...
  test('weapon to +1', async ({ page }) => {
    ...
    await expect(page.locator('[id="🏠page-demo__X2"]')).toContainText('請選擇一種武器。');
    await page.locator('[id="🧱StatusEquips__Equip"]').first().click();
    await expect(page.locator('id=🏠page-demo__X2')).toContainText('+0 瑟魯基之劍 一瞬間發出 藍色的 光芒。');
  });
  ...
  test('weapon to -1', async ({ page }) => {
    ...
    await expect(page.locator('[id="🏠page-demo__X2"]')).toContainText('請選擇一種武器。');
    await page.locator('[id="🧱StatusEquips__Equip"]').first().click();
    await expect(page.locator('id=🏠page-demo__X2')).toContainText('+0 瑟魯基之劍 一瞬間發出 黑色的 光芒。');
  })
})